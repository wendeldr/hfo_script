...
# parser = argparse.ArgumentParser(description='Process EDF files for HFO detection')
# parser.add_argument("input_file", help="Input EDF file path")
# parser.add_argument("-v", "--verbosity", type=int, choices=[0, 1, 2, 3], 
#                     default=1, help="Verbosity level (0=None, 1=Low, 2=Medium, 3=High)")
...
edf_path = "/Users/dan/Desktop/hfo_script/098_Baseline.EDF"
verbosity = 0
dry_run = False
overwrite = False
...
hfo_band = (80, None)
window_size_sec = 0.03
overlap = 0.25
threshold = 5
n_jobs = 50
output_path = None
save_type = "both" # can be counts, raw, or both; default is both
...
if hfo_band[1] is None:
    hfo_band = (hfo_band[0], int(sf // 2)-1)
    if verbosity >= VERBOSITY_LOW:
        print("HFO band set to (l_freq, Nyquist - 1)\n")
    detector_kwargs['filter_band'] = hfo_band

if max(hfo_band) >= int(sf // 2):
    print("ERROR:")
    print(f"Defined HFO band ({hfo_band[0]}-{hfo_band[1]} Hz) is at or above Nyquist frequency ({sf//2} Hz)")
    print(f"Upper band limit must be set strictly below Nyquist frequency. (Max upper limit: {int(sf//2)-1} Hz)")
